 ...
 *  Update Date: 2023/06/12 08:00:18
...
Torch::Torch(const sf::Vector2f &pos) { this->setPos(pos); }

void Torch::init()
{
    int spriteSize = 8;
    this->texture.loadFromFile(
        fs::path(File::getResPath() / "images/spritesheet_torch.png").string());
    ...
    this->animationManager.addAnimation("Flickering", 0.07, 0, 0, 2, 0,
...
void Torch::render(sf::RenderWindow &window) { window.draw(this->sprite); }
