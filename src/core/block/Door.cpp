 ...
 *  Update Date: 2023/06/12 06:38:24
...
Door::Door(Player *player, const sf::Vector2f &pos)
    : player(player), closed(true)
{
    this->setPos(pos);
}
...
void Door::init()
{
    int spriteSize = 8;
    this->textureClosed =
        sf::IntRect(spriteSize * 2, spriteSize * 1, spriteSize, spriteSize);
    this->textureOpened =
        sf::IntRect(spriteSize * 3, spriteSize * 1, spriteSize, spriteSize);
    this->textureTest = sf::IntRect(0, spriteSize * 2, spriteSize, spriteSize);
...
void Door::render(sf::RenderWindow &window) { window.draw(this->sprite); }

Json Door::toJson() const
{
    Json j;
    j["type"] = "Door";
    j["closed"] = this->closed;
    j.update(Block::toJson());
    return j;
}

void Door::fromJson(const Json &json)
{
    this->closed = json["closed"].get<bool>();
    Block::fromJson(json);
}
